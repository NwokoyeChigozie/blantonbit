    ...
    $email = mysqli_real_escape_string($link, $_POST['email']);
       ...
       if (empty($email)) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($count < 1) {
                echo '<div class="alert alert-danger" style="border-radius:3px;text-align:center;background-color:#E23D28;color:#fff;padding:10px 85px;margin-top:0px">The email address you have entered is not registered to Blanton. Please <a href="./?a=signup" style="color:#6051da">Signup</a></div>';
                exit;
            }
...
//            $registered_at = date("M-d-Y h:i:s A", time());
            $elapse_at = time() + 86400;
                $sql= "INSERT INTO `password_recovery`(`email`,`sr`, `elapse_time`, `count`) VALUES ('$email','$sr','$elapse_at','0')";
        ...
        if(mysqli_query($link, $sql)){
            ...
            $link = "https://$blanton_domain/?a=forgot_password&sr=" . $sr . "&pri=" . $pr_id . "&email=" . $email . "&et=" . $elapse_at;
            ...
            // echo $link;
            $from_email = "support@$blanton_domain";
            $title = "Password Reset";
            $small_text_detail = "You have requested a Password change";
            $button_name = "Reset Password";
            include('forgot_password_email.php');
...
//          $from = "gregoflash05@gmail.com"  
        ...
        $headers .= 'From: AstroFxcInvestment <support@' . $blanton_domain. '>' . "\r\n";
