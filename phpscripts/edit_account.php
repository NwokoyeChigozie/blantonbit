                        ...
                        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            if($count >= 1 && $email != $main_email){
                                ...
                                if(empty($password)){
                                        ...
                                        $sql_t = "UPDATE `users` SET `full_name`='$full_name', `email`='$email', `phone`='$phone',`bitcoin_wallet_address`='$bitcoin_wallet',`ethereum_wallet_address`='$ethereum_wallet' WHERE `id`= '$u_id'";    
                                        if(mysqli_query($link, $sql_t)){
                                            ...
                                            echo "<script>
                                                function navigate(){
                                                window.location = './?a=edit_account';
                                                }
                                    ...
                                    if (strlen($password)<8) {
                                        ...
                                        $sql_t = "UPDATE `users` SET `full_name`='$full_name', `email`='$email', `phone`='$phone', `bitcoin_wallet_address`='$bitcoin_wallet', `password`='$password',`ethereum_wallet_address`='$ethereum_wallet' WHERE `id`= '$u_id'";    
                                        if(mysqli_query($link, $sql_t)){
                                            echo '<div class="alert alert-success" style="border-radius:3px;text-align:center;background-color:green;color:#fff;padding:10px 85px;margin-top:0px">Account Successfully Updated</div>';
                                                echo "<script>
                                                function navigate(){
