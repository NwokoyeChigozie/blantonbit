...
if ($withdrawal_type == 5){
  $purpose = "Referral Commission";
  $sql = "UPDATE `users` SET `total_referal_commission`='0' WHERE `id`= '$withdrawal_u_id'"; 
  if(!$result = mysqli_query($link, $sql)){
    // print_r(mysqli_error($link));
    echo "<div class='alert alert-error'>Error occured updating user, Try again later</div>";
    exit;
  }
}
...
$sql = "UPDATE `history` SET `status`='completed' WHERE `tid`= '$w_id' AND type = '$purpose' AND u_id = '$withdrawal_u_id'";  
  if(!$result = mysqli_query($link, $sql)){
    echo "<div class='alert alert-error'>Error occured updating history, Try again later</div>"; 
       header("Refresh:2; url=withdrawal_requests.php");
      exit;
  }  
